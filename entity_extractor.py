...
def sent2features(sent):
	treemodel = 0;
	reval = [word2features(sent[1], i) for i in range(len(sent[1]))];
	return reval;

def sent2featuresWithSent(sent, strsent):
	return [word2features(sent, i, 0, 0) for i in range(len(sent))];
...
def evaluation(y_true, y_pred, bioset):
	...
	class_indices = {cls: idx for idx, cls in enumerate(bioset)};
